...
const createProduct = asyncHandler(async (req, res) => {
  ...
  const { name, unit, price, mrp, stock, tags, categoryId, description, brand } = req.body;
  if (!name || price === undefined) {
    return res.status(400).json({ success: false, message: 'Product name and price are required' });
  }

  const nameKeywords = Array.isArray(tags) ? tags : [];
  ...
  const product = await Product.create({
    ...
    brand,
    tags: nameKeywords,
    nameKeywords: [name.toLowerCase(), ...nameKeywords.map((t) => t.toLowerCase())],
  });

  res.status(201).json({ success: true, product });
});

// PUT /api/products/:id (storeOwner)
const updateProduct = asyncHandler(async (req, res) => {
  ...
  const { name, unit, price, mrp, stock, tags, categoryId, description, brand, isAvailable } = req.body;
